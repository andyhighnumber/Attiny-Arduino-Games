...
char screenText[LASTLINE-FIRSTLINE+1][LINELENGTH];
...
void newLine() {
    ...
    if (LCDline == LASTLINE) {scrollNow = 1; oldLine = LASTLINE-1;} else {LCDline++; scrollNow = 0;}
    ...
    if (decodedCharacter > ' ') {   
      for (int i = 0; i < currentWordLength; i++) {
        ...
        sendToLCD();                        
        screenText[oldLine-FIRSTLINE-1][LINELENGTH-i] = ' '; 
      }  
