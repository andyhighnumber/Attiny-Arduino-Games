...
0x7F,0x3F,0x3F,0x3E,0x1E,0x1E,0x06};
...
void displayTitle(void) {
  int incr = 0;
  byte lengths[3] = {22,35,23}; // to save prog memory, each line is only drawn as far as the first blank byte

  for(int lxn = 3; lxn < 6; lxn++) {
    ssd1306_setpos(86,lxn); 
    ssd1306_send_data_start();
    for(int lxn2 = 0; lxn2 < lengths[lxn-3]; lxn2++) {
