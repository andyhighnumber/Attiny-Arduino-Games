...
byte ghostStart[3] = {28,17,39};  // The vertical location of whete the ghosts start
...
void loop() { 
  ...
  // The character set is seriously compromised because I've had to truncate the ASCII table
  // to release space for executable code. Some letters are missing and stuff - check font6x8AJ3.h for details
  ssd1306_char_f6x8(20, 2, " P A C M A N");
  ssd1306_char_f6x8(24, 4, "andy jackson"); // see comments above !
  ...
  ssd1306_char_f6x8(20, 6, " inspired by");
  ssd1306_char_f6x8(20, 7, "webboggles.com");

  ssd1306_setpos(34,0); 
  ssd1306_send_data_start();
  for (byte lxn = 0; lxn < 8; lxn++) {
      ssd1306_send_byte(pgm_read_byte(&pacOpen[0][lxn]));
  }
  ssd1306_send_data_stop();                    

  for (byte lxn = 46; lxn < 81; lxn+=8) {
      ssd1306_setpos(lxn,0); 
      ssd1306_send_data_start();
      ssd1306_send_byte(B00010000);
      ssd1306_send_data_stop();                    
  }
  ...
  for(byte c = 0; c < 8; c++) {
    ...
    for (byte r = 0; r < 128; r++) {
      ...
      if (r < 7) {
        ...
        if (c == 3 && lives > 3) out |= (pgm_read_byte(&PAClives[r]));
        if (c == 4 && lives > 2) out |= (pgm_read_byte(&PAClives[r]));
      ...
      for (byte lxn = startP; lxn < stopP; lxn++) {        
        if (pillsActive[lxn] == 1) { // this pill is active
          if (pgm_read_byte(&pillLocations[lxn][0]) > locations[3][0] && pgm_read_byte(&pillLocations[lxn][0]) < locations[3][0] + 8 && pgm_read_byte(&pillLocations[lxn][1]) > locations[3][1] && pgm_read_byte(&pillLocations[lxn][1]) < locations[3][1] + 8) {pillsActive[lxn] = 0; pillsEaten++;score++; break;}
      ...
      if (locations[3][1] == 51) {
        if (locations[3][0] < 14 && bigPillsActive[0] == 1) {bigPillsActive[0] = 0; powerUp = 1; powerCounter = 0;}
        if (locations[3][0] > 111 && bigPillsActive[1] == 1){bigPillsActive[1] = 0; powerUp = 1; powerCounter = 0;}
      } 
...
byte hitGhosts(void){
  ...
  for (int lxn = 0; lxn < 3 ;lxn++) {
    if ((locations[lxn][0] > locations[3][0] - 8) && (locations[lxn][0] < locations[3][0] + 8) && (locations[lxn][1] > locations[3][1] - 8) && (locations[lxn][1] < locations[3][1] + 8)) {
      if (powerUp == 1) {
        locations[lxn][0] = 71;
        locations[lxn][1] = ghostStart[lxn];
        ghostsActive[lxn] =0;
        for (int i = 700; i>200; i = i - 50){
          beep(30,i);
        }
...
void playPacman(){
  ...
  while(lives >= 0) {
    ...
    for (byte lxn2 = 0; lxn2 < 4; lxn2 ++) {
      if (locations[lxn2][1] <= 0) locations[lxn2][1] = 56; else if (locations[lxn2][1] >= 56) locations[lxn2][1] = 0;
    }
